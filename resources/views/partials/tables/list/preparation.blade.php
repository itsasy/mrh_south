            ...
            <table class="table table-hover table-inverse table-responsive">
                ...
                <tbody>
                    ...
                    <tr class="text-center">
                        ...
                        @if($index == 0)
                        <td class="text-nowrap" rowspan={{count($sample->ChoiceTestSample)}}>{{$sample->fecha_registro}}
                        </td>
                        @endif
                        @if($index == 0)
                        ...
                        <td rowspan={{count($sample->ChoiceTestSample)}}>
                             @if($sample->estado_qda !== null)
                            <a role="button" @if($sample->estado_qda == 0) 
                                ...
                                @else style="color:green !important;"@endif><i class="fa fa-check-circle fa-2x"></i>
                            </a>
                            @endif
                        </td>
                        @endif
                        @if($index == 0)
                        <td rowspan={{count($sample->ChoiceTestSample)}}>
                          @if($sample->estado_PC !== null)
                            <a role="button" @if($sample->estado_PC == 0)
                                href="{{route('preparation.create', ['preparation'=> $sample->id_muestra, 'type' => 'Perfil de consumidores', 'orthogonal_code' => null])}}"
                        ...
                        <td>
                            <a rel="noopener noreferrer" role="button" @if($csample->excel_respuestas_duo_trio != null)
                                href="{{route('downloadExcel', [$csample->excel_respuestas_duo_trio])}}"
